...
public class SslUtil {
    ...
    public static SSLSocketFactory getSocketFactory(final String rootCaCertFile,
        ...
        try (PEMParser clientPrivateKeyParser =
            ...
            RobotManagerForm.error(String.format("Failed to load private key file at {%s} with error: {%s}",
    ...
    private static X509Certificate getX509Certificate(final String certFileName) {
        try (PEMParser certParser = new PEMParser(new FileReader(certFileName))) {
            ...
            RobotManagerForm.error(String.format("Failed to load certificate file at {%s} with error: {%s}",
                    certFileName, e.getMessage()));
            return null;
        }
    }
}